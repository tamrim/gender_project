...
gen adults = age_20_29+age_30_39
bys Gubernia Gender: egen total_children_grown_g = sum(adults)
bys Gubernia: egen total_children_grown = sum(adults)


gen female_prop_1897 = total_children_grown_g/total_children_grown if Gender=="Female"
drop if female_prop_1897==.

rename Gubernia Province
keep Province female_prop_1897
duplicates drop
tempfile census
save `census'
...
merge 1:1 Province using `census'

label var female_prop_1897 "Female/Male ratio,  "

reg female_prop_1897  death_female_to_male, r 
est store firststage_1
estadd local fixed "no" , replace

xi: reg female_prop_1897  death_female_to_male i.Region, r
est store firststage_2
estadd local fixed "yes" , replace

esttab firststage_1 firststage_2 using "$tables/firststage_1872_1879.tex", style(tex) label cells(b(fmt(3) star) se(fmt(2) par)) drop(_IReg*) collabels(none) stats(fixed N r2, label("Region Fixed Effects" "N" "R2") fmt(str3 0 %9.3f)) star(* 0.1 ** 0.05 *** 0.01) addnote("* p$<$0.1, ** p$<$0.05, *** p$<$0.01") replace
